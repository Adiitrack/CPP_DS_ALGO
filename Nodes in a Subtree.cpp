...
Node * locateSubNode(Node* root, int u, int idx, int size){
  if(idx==u)return root;
  if(idx>u)return NULL;
  if(idx<u){
    Node * subNode;/* = locateSubNode(root->children[0],u,idx*2,size);
    if(subNode==NULL){
      subNode = locateSubNode(root,u,idx*2+1,size);
    }*/
    for(int i=0;i< root->children.size();i++){
      subNode = locateSubNode(root->children[i],u,idx*2+i,size);
      if(subNode!=NULL){
        break;
      }
    }
...
void countr (Node* root, char c,int &count,string &s){
  if(root==NULL){
    return;
  }else if(s[root->val-1] == c){
    count++;
  }
  for(int i=0;i<root->children.size();i++){
     countr(root->children[i],c,count,s);
   }
}
...
vector<int> countOfNodes(Node* root, vector<Query> queries, string s) {
  ...
  for(int i=0;i<queries.size();i++){
    int count = 0;
    Node * base = locateSubNode(root,queries[i].u,1,size);
    cout<<base->val<<" ";
    countr(base,queries[i].c,count,s);
    NodeCounter.push_back(count);                           
  }
